...
class Maps2ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //MARK: Posisi Camera
        mapView.camera = GMSCameraPosition.camera(withLatitude: 1.1208181525740293, longitude: 104.04862034301658, zoom: 16.5)
        mapView.isMyLocationEnabled = true
        mapView.settings.myLocationButton = true
        mapView.settings.compassButton = true
        // Posisi Camera (End)
        
        let position = CLLocationCoordinate2D(latitude: 1.1208181525740293, longitude: 104.04862034301658)
        let marker = GMSMarker(position: position)
        marker.title = "KFC Batam Center"
        marker.map = mapView
